...
void Affiche_Ensemble_ord(uint64_t *pt, int length, int n);
...
/* GÉNÉRATION DE NOMBRES ALÉATOIRES DE TAILLE 64 BITS */
...
uint64_t GJBExtraction(uint64_t* S, int d, uint64_t Taille, uint64_t**pt, uint64_t*** L){
    /*
    ENTRÉE: 
    - un sous-ensemble S de F_2^n, préalablement défini
...
int main(int argc, char const *argv[]){
    ...
    double temps0 = ((double)(clock() - sec)/CLOCKS_PER_SEC);
    ...
    */
    ...
    /* Affichage du temps de calcul de la fonction seule */
    double temps1 = ((double)(clock() - sec)/CLOCKS_PER_SEC) - temps0;
    ...
    printf("Le nombre de GJB de dimension %d dans S vaut %d\n",d,x);
    for(int j=0;j<x;j++){
        printf("Le %d-ème sous espace de dimension %d est : ",j+1,d);
        Affiche_Ensemble_ord(L[0][j],d,n);
    }

    /* Affichage du temps de calcul de l'affichage des résultats de la fonction */
    double temps2 = ((double)(clock() - sec)/CLOCKS_PER_SEC) - temps1 - temps0;
    printf("Le temps d'affichage des résultats de GJBExtraction est: %f secondes\n", temps2);
}
